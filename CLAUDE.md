...
- `lua/custom/init.lua` - Custom settings loaded at end of init.lua: window resize keymaps, theming (rose-pine via OSC11 detection), custom commands (`:Ask`, `:Diff`, `:Diffs`, `:ReloadConfig`), colorcolumn, save keymaps, exrc support. Also defines the `add_directory` global for project `.nvim.lua` files: shows dotfiles except `.git` in telescope find_files/live_grep and neo-tree, with optional search roots.
